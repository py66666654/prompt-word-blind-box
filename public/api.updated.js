...
class Api {
    ...
    // 按类型获取随机提示词
    async getRandomPromptByType(typeId) {
        return this.request(`/prompts/random/type/${typeId}`);
    }
    ...
    async getPromptTypes() {
        return this.request('/prompts/types');
    }

    // 获取所有稀有度等级
    async getRarityLevels() {
        return this.request('/prompts/rarity-levels');
    }

    // 根据分类ID获取提示词
    async getPromptsByCategory(categoryId, page = 1, typeId = '', rarityId = '') {
        let url = `/prompts/category/${categoryId}?page=${page}`;
    ...
    // 生成AI提示词
    async generateAIPrompt(type = '') {
        return this.request('/prompts/generate/ai', {
            method: 'POST',
